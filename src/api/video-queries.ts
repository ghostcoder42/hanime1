...
export const useVideoDetail = createQuery<VideoDetail, { id: string }, Error>({
  ...
  // The fetcher already times out after 15s; with the default 2 retries a
  // dead network keeps the watch screen spinning for ~48s before the error
  // screen appears, which reads as a freeze. Fail once, let the user retry.
  retry: 0,
});
